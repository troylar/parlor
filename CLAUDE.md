...
- **`services/vector_index.py`** — usearch-based vector similarity search. `VectorIndex` class: thread-safe wrapper around usearch `Index` with string-to-int key mapping (SHA-256 truncated to 63 bits), add/remove/search/save/clear/rebuild_key_map. `VectorIndexManager`: manages two `VectorIndex` instances (messages + source_chunks), handles initialization, rebuild from SQLite metadata, and persistence. `has_vector_support()` checks usearch availability. Graceful degradation: all consumers accept `vec_index=None`
- **`services/storage.py`** — SQLite DAL with column-allowlisted SQL builder, parameterized queries, UUID IDs. Vector storage via usearch (graceful degradation when usearch not installed). Source CRUD, tags, groups, space CRUD and linking, text chunking, embeddings. Token usage tracking. Conversation slugs. `search_similar_messages()` supports optional `conversation_type` and `space_id` filters for type- and space-scoped semantic search. `search_similar_source_chunks()` supports `space_id` filter with over-fetch and post-filter strategy
...
- **`services/rag.py`** — RAG pipeline: embed query, search similar messages/source chunks via usearch, filter by threshold, deduplicate, trim to token budget. Gracefully degrades. `retrieve_context()` accepts `space_id` for scoped retrieval. `RetrievedChunk` includes `conversation_type` field; `format_rag_context()` annotates chunks with type labels (`[note]`, `[doc]`)
