...
- **`cli/renderer.py`** — Rich terminal output: thinking spinner, plan checklist, inline diffs, tool call dedup, subagent rendering. Focus & Fold: turn-level tool batch grouping with in-place progress ticker (`FoldGroup`, `_update_fold_ticker()`), compact completion summaries, and Ctrl-O expand/collapse. `use_stdout_console()` for REPL-compatible mode via raw stderr fd
