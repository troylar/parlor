...
- **`cli/layout.py`** — Full-screen terminal layout with HSplit structure: persistent header showing model/directory/git branch, scrolling output pane with mouse-scroll support and auto-scroll-to-bottom, separator lines, status footer, and input area. `OutputControl` auto-scrolls output to latest content via cursor positioning; checkpoint/truncate API (`set_checkpoint()`, `truncate_to()`) enables in-place replacement of output fragments for plan/thinking blocks. `AnteroomLayout` class builds the layout tree. `OutputPaneWriter` provides async write interface for streaming output with debounced invalidation (`debounce_interval`, `force_invalidate()`) to batch repaints. `format_header()` renders header text with metadata. `create_anteroom_style()` defines Rich styles for layout elements
