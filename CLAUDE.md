...
- **`services/embedding_worker.py`** — Background worker that processes unembedded messages asynchronously, runs on a configurable interval. Also processes pending source chunks via `process_pending_source_chunks()` (same pattern as message embedding but queries `get_unembedded_source_chunks()` and stores via `store_source_chunk_embedding()`). `embed_source()` provides inline embedding on source creation
