...
- **`services/workflow_engine.py`** — Domain-neutral workflow engine: load YAML, execute steps (runner/gate/loop), concurrency locks, heartbeat, resume
- **`services/workflow_storage.py`** — CRUD for workflow runs, steps, events, approvals, locks. Atomic lock via IntegrityError
- **`services/workflow_runners.py`** — Runner registry + execution. Agent runners via agent_loop; opaque runners via subprocess
- **`services/workflow_hooks.py`** — Notification hooks: webhook (egress-allowlisted) and Unix socket with bounded drain
...
- **`routers/workflows.py`** — Read-only workflow monitoring API: list definitions, list/filter runs, run detail with steps, event history
...
- **`cli/workflow_cli.py`** — CLI handlers for `aroom workflow` subcommands (run/status/list/history/resume/cancel). Progress callback, EventBus cross-process delivery
