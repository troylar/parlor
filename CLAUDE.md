...
- **`__main__.py`** — CLI entry point. Argparse dispatch: `init`, `config`, `chat`, `exec`, `db`, `usage` subcommands. Global flags: `--version`, `--test`, `--allowed-tools`, `--approval-mode`, `--port`. Chat flags: `--trust-project`, `--no-project-context`, `--plan`. Usage subcommand: `--period` (day/week/month/all), `--conversation`, `--json`
