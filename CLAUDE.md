...
- **`services/artifact_storage.py`** — Artifact CRUD with version tracking. `list_artifacts()` supports `attached_only` and `space_id` filtering
- **`services/artifact_registry.py`** — In-memory artifact index with 6-layer precedence. Space-scoped loading via `load_from_db(space_id=)`
...
#### CLI Modules
- **`cli/repl.py`** — Main REPL loop with prompt_toolkit. Orchestrates: system prompt building, space context, trust verification, plan mode, skill auto-invocation. Imports shared command metadata (names, descriptions, subcommand completions) from `commands.py` for autocomplete and help rendering; full command dispatch still uses the inline `if/elif` chain (Phase 2 wiring tracked in #805). Delegates agent turns to `agent_turn.py`, events to `event_handlers.py`. Manages `/pack`, `/space`, `/artifact`, `/config`, `/reprocess` commands. `/instructions` alias for `/conventions`. Space auto-detection from cwd with parent walk-up. Skill args sanitized with `sanitize_trust_tags()` and delimited with `<skill_args>` tags for injection defense
...
- **`cli/commands.py`** — Shared slash-command engine: single source of truth for command metadata, parsing, and dispatch. `CommandKind` literal type exhaustively classifies command intent (30+ kinds: exit, conversation management, display, skills, spaces, artifacts, packs, MCP, planning, config, file operations). `ParsedSlashCommand`, `CommandContext`, and `CommandResult` (frozen dataclass) are pure, effect-free models. `execute_slash_command()` returns typed results for adaptation by CLI/Textual/web interfaces. Subcommand dispatchers (`_dispatch_space()`, `_dispatch_pack()`, `_dispatch_artifact()`, `_dispatch_plan()`, `_dispatch_mcp()`, `_dispatch_config()`) route subcommands to the correct intent. Helper builders (`build_help_markdown()`, `build_tools_markdown()`, `build_skills_markdown()`) render display-ready output. Parses skill invocation via `SkillRegistry.resolve_input()` as fallback. `get_builtin_names()` exports canonical command set for skill collision detection.
